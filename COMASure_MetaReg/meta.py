...
class Meta(nn.Module):
  ...
  def __init__(self, 
            ...
            update_lr, meta_lr, meta_test_update_lr, meta_reg_lr,

            support_set_size, query_set_size,

            update_step, update_step_metareg_a, update_step_metareg_b,
    ...
    self.meta_reg_lr = meta_reg_lr # MetaReg lr

    self.support_set_size = support_set_size
    self.query_set_size = query_set_size

    self.update_step = update_step # Number of gradient updates at Meta-Training time
    self.update_step_metareg_a = update_step_metareg_a
    self.update_step_metareg_b = update_step_metareg_b
    ...
    self.metareg = MetaReg(self.meta_reg_lr, self.layer_sizes[-2], self.layer_sizes[-1]).cuda()
    self.meta_opt = torch.optim.Adam(self.net.parameters(), lr=self.meta_lr)
    self.metareg_opt = torch.optim.Adam(self.metareg.parameters(), lr=self.meta_reg_lr)
  ...
  def compute_loss(self, result, out, fast_weights, C, H, W, mode):
    ...
    target_layers = ((len(self.layer_sizes)-1)*2-2,(len(self.layer_sizes)-1)*2-1)
    target_layers_paras_list = []
    if self.use_regularization:
      counter = 0
      l2_loss = 0.0
      for param in fast_weights:
        if counter in target_layers:
          target_layers_paras_list.append(torch.flatten(param))
        l2_loss += torch.norm(param)**2
        counter += 1
      loss = loss + self.reg_coeff * l2_loss
      metareg_loss = self.metareg(torch.cat([target_layers_paras_list[0], target_layers_paras_list[1]], dim=0).unsqueeze(0))
      loss = loss + metareg_loss

    return loss, metareg_loss
  ...
  def compute_meta_loss(self, tasks, tasks_with_imgs, tasks_with_imgs_patchs, fast_weights):
    ...
    for out in meta_train_query_target[0]:
      ...
      loss, loss_metareg = self.compute_loss(result, out.cuda(), fast_weights, meta_train_query_C, meta_train_query_H, meta_train_query_W, "query")
      total_loss = total_loss + loss
      total_loss_metareg = total_loss_metareg + loss_metareg
    ...
    return total_loss/(self.query_set_size*self.patch_count), total_loss_metareg/(self.query_set_size*self.patch_count), \
  ...
  def execute_grad_step(self, tasks, tasks_with_imgs, tasks_with_imgs_patchs, fast_weights, step):
    ...
    for out in meta_train_support_target[0]:
      if step == 0:
          ...
          loss, _ = self.compute_loss(result, out.cuda(), self.net.parameters(), meta_train_support_C, meta_train_support_H, meta_train_support_W, "support")
  ...
  def execute_grad_step_metareg(self, tasks, tasks_with_imgs, tasks_with_imgs_patchs, fast_weights):
  ...
  def meta_train(self, all_tasks=None):
    ...
    total_meta_loss = 0
    total_metareg_loss = 0

    for i in task_num_list: # Outer loop
      
      fast_weights = list(map(lambda p: p[0], self.net.parameters())) # Initialize as original network paras (theta)
      ...
      meta_loss, metareg_loss, psnr_avg, ssim_avg = self.compute_meta_loss(tasks, tasks_with_imgs, tasks_with_imgs_patchs, fast_weights)
      total_meta_loss = total_meta_loss + meta_loss
      total_metareg_loss = total_metareg_loss + metareg_loss
    ...
    return np.mean(np.array(psnr_avg_list)), np.mean(np.array(ssim_avg_list)), \
        total_meta_loss/len(task_num_list), total_metareg_loss/len(task_num_list)

  def meta_train_metareg(self, task_a, task_b):

    self.metareg_opt.zero_grad()
    self.meta_opt.zero_grad()
    
    fast_weights = list(map(lambda p: p[0], self.net.parameters()))

    task_a_with_imgs = assign_img_data_to_tasks(task_a, self.dataset_name, self.img_name_list_start, self.img_name_list_end, self.img_type, self.downscaling_factor, self.downscaling_method, True)
    task_a_with_imgs_patchs = generate_patches_meta_support_query(len(task_a), self.query_set_size, self.support_set_size, task_a, task_a_with_imgs, self.patch_size_lr, self.patch_count, self.downscaling_factor)

    for step in range(self.update_step_metareg_a):
      fast_weights = self.execute_grad_step(task_a, task_a_with_imgs, task_a_with_imgs_patchs, fast_weights, step) # The first step of task a will still need to go through original "execute_grad_step".
    
    task_b_with_imgs = assign_img_data_to_tasks(task_b, self.dataset_name, self.img_name_list_start, self.img_name_list_end, self.img_type, self.downscaling_factor, self.downscaling_method, True)
    task_b_with_imgs_patchs = generate_patches_meta_support_query(len(task_b), self.query_set_size, self.support_set_size, task_b, task_b_with_imgs, self.patch_size_lr, self.patch_count, self.downscaling_factor)

    for step in range(self.update_step_metareg_b):
      fast_weights = self.execute_grad_step_metareg(task_b, task_b_with_imgs, task_b_with_imgs_patchs, fast_weights)
    
    meta_loss, _, _, _ = self.compute_meta_loss(task_b, task_b_with_imgs, task_b_with_imgs_patchs, fast_weights)
    meta_loss.backward()
    self.metareg_opt.step()    
  ...
  def meta_test_with_ground_truth(self, coords_base, target_base, meta_test_support_H, meta_test_support_W, meta_test_support_C, meta_test_query_H, meta_test_query_W, meta_test_query_C, update_step_test, meta_test_evaluation_steps, coords_hr, target_hr):
    ...
    loss, loss_metareg = self.compute_loss(result, target_hr.cuda(), net.parameters(), meta_test_query_C, meta_test_query_H, meta_test_query_W, "query")

    del net

    return sr_interpolate_results, best_step, best_PSNR_value, best_SSIM_value, steps, loss, loss_metareg
  ...
  def meta_test_without_ground_truth(self, coords_base, target_base, meta_test_support_C, meta_test_support_H, meta_test_support_W, update_step_test, meta_test_evaluation_steps, coords_hr):
    ...
    update_lr = copy.deepcopy(self.meta_test_update_lr)
    gamma = copy.deepcopy(self.gamma)
    opt = torch.optim.Adam(net.parameters(), lr=update_lr) # DReg is not tuned at meta-testing time
    scheduler = torch.optim.lr_scheduler.ExponentialLR(opt, gamma=gamma)
