...
class Meta(nn.Module):
  ...
  def compute_loss(self, result, out, fast_weights, C, H, W, mode):
    ...
    # DReg Loss
    dreg_loss = self.vgg(result.reshape(H, W, C).permute(2,0,1).unsqueeze(0), mode, self.patch_size_lr, self.downscaling_factor)
    loss = loss + dreg_loss 

    return loss, dreg_loss
  ...
  def compute_meta_loss(self, tasks, tasks_with_imgs, tasks_with_imgs_patchs, fast_weights):
    ...
    for out in meta_train_query_target[0]:
      ...
      loss, loss_dreg = self.compute_loss(result, out.cuda(), fast_weights, meta_train_query_C, meta_train_query_H, meta_train_query_W, "query")
      total_loss = total_loss + loss
      total_loss_dreg = total_loss_dreg + loss_dreg
    ...
    return total_loss/(self.query_set_size*self.patch_count), total_loss_dreg/(self.query_set_size*self.patch_count), \
  ...
  def meta_train(self, all_tasks=None):
    ...
    for i in task_num_list: # Outer loop
      ...
      meta_loss, dreg_loss, psnr_avg, ssim_avg = self.compute_meta_loss(tasks, tasks_with_imgs, tasks_with_imgs_patchs, fast_weights)
      total_meta_loss = total_meta_loss + meta_loss
      total_dreg_loss = total_dreg_loss + dreg_loss
    ...
    return np.mean(np.array(psnr_avg_list)), np.mean(np.array(ssim_avg_list)), \
        total_meta_loss/len(task_num_list), total_dreg_loss/len(task_num_list)
  ...
  def meta_test_with_ground_truth(self, coords_base, target_base, meta_test_support_H, meta_test_support_W, meta_test_support_C, meta_test_query_H, meta_test_query_W, meta_test_query_C, update_step_test, meta_test_evaluation_steps, coords_hr, target_hr):
    ...
    loss, loss_dreg = self.compute_loss(result, target_hr.cuda(), net.parameters(), meta_test_query_C, meta_test_query_H, meta_test_query_W, "query")

    del net

    return sr_interpolate_results, best_step, best_PSNR_value, best_SSIM_value, steps, loss, loss_dreg
